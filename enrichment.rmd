...
xgr_ora_plots <- function(res = paste0(results_dir, "deseq2_results_annotated.tsv"), pathways = "DO", FC = 1, pvalue=pvalue_ORA) {
  
  results_annotated <- read.delim(res)
  if (FC < 0) {xgr_data <- results_annotated %>%
    dplyr::filter(padj < pvalue_ORA, log2FoldChange < FC
                  #!is.na(ENSEMBL)
    ) 
  }
...
background <- results_annotated %>% 
  dplyr::filter(!is.na(hgnc_symbol) & !duplicated(hgnc_symbol)) %>%
  dplyr::pull(hgnc_symbol)
ontology <- list("KEGG" = "MsigdbC2KEGG", "Reactome" = "MsigdbC2REACTOME", "DO" = "DO")
eTerm <- xEnricherGenes(data=data, background=background, ontology= ontology[[pathways]], ontology.algorithm = "lea")
...
xgr_reactome_up <- xgr_ora_plots(res, "Reactome", 1, pvalue_ORA)
xgr_do_up <- xgr_ora_plots(res, "DO", 1, pvalue_ORA)
xgr_kegg_up <- xgr_ora_plots(res, "KEGG", 1, pvalue_ORA)

print("DOWN regulated genes")

xgr_reactome_down <- xgr_ora_plots(res, "Reactome", -1, pvalue_ORA)
xgr_do_down <- xgr_ora_plots(res, "DO", -1, pvalue_ORA)
xgr_kegg_down <- xgr_ora_plots(res, "KEGG", -1, pvalue_ORA)
...
TF_ranking_down_wilcoxon %>% head(50) %>% bind_rows(TF_ranking_up_wilcoxon %>% head(50)) %>% filter(TF %in% decoupleR_deg_top50_variable_TFs) %>% View()
...
CeTF_networks <- list()
names(CeTF_network) <- c(Test, Control)
...
Volcano_Plots(de_seq = DeSeq2_return$de_seq, 
              ...
              filter_specific = c(CeTF_networks$Carfilzomib, CeTF_networks$WT),
              FC = 1)
...
names(enrichment_table) <- c("Biogrid Reactome Pathways", "Representative fuzzy clustering", "Representative hierarchical clustering")

score_matrix <- list()

for (i in 1:length(enrichment_table)) {
  
  score_matrix[[i]] <- score_terms(enrichment_table = enrichment_table[[i]],
                            exp_mat = exp_mat,

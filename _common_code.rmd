...
pr_input <- results_annotated %>%
  select(Gene.symbol = hgnc_symbol, 
         logFC	= log2FoldChange, 
         adj.P.Val = padj) %>% 
  filter(!is.na(adj.P.Val), !is.na(Gene.symbol), !is.na(logFC))




pathfindR_output <- run_pathfindR(
  pr_input,
  gene_sets = "Reactome",
  min_gset_size = 5,
  max_gset_size = 500,
...
enrichment_chart(result_df = pathfindR_output, 
                 top_terms = 15)
...
grn = hLICORN(cemitool_input, TFlist=HumanTF)
